...
def left(hand_landmarks):
    middle_tip_y =  hand_landmarks.landmark[mp_hands.HandLandmark.MIDDLE_FINGER_TIP].x
    wrist =  hand_landmarks.landmark[mp_hands.HandLandmark.WRIST].x

    if middle_tip_y-wrist>50:
        return True
    return False
        ...
        for hand_landmarks in results.multi_hand_landmarks:
            ...
            if forward(hand_landmarks):
                ...
                if left(hand_landmarks):
                    cv2.putText(img, 'going left', (50, 50), cv2.FONT_HERSHEY_SIMPLEX, 1, (0, 255, 0), 2, cv2.LINE_AA)
                    if not gesture_detected:
                        arduino.write(b'l')  
                        gesture_detected = True
            ...
            if reverse(hand_landmarks):
                cv2.putText(img, 'going backwards', (50, 50), cv2.FONT_HERSHEY_SIMPLEX, 1, (0, 255, 0), 2, cv2.LINE_AA)
                if not gesture_detected:
                    arduino.write(b'r')  
                    gesture_detected = True'''
    if not gesture_detected:
